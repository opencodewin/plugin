...
namespace BluePrint
{
struct Orthographic2EquidistanceNode final : Node
{
    BP_NODE_WITH_NAME(Orthographic2EquidistanceNode, "Orthographic2Equidistance", "CodeWin", NODE_VERSION, VERSION_BLUEPRINT_API, NodeType::External, NodeStyle::Default, "Filter#Video#Fisheye")
    Orthographic2EquidistanceNode(BP* blueprint): Node(blueprint) { m_Name = "Orthographic to Equidistance"; m_HasCustomLayout = true; m_Skippable = true; }
    ~Orthographic2EquidistanceNode()
    {
    ...
    FlowPin Execute(Context& context, FlowPin& entryPoint, bool threading = false) override
    {
        ...
        if (!mat_in.empty())
        {
            ...
            {
                if (m_filter) { delete m_filter; m_filter = nullptr; }
                m_filter = new ImGui::Orthographic2Equidistance_vulkan(gpu);
            }
            if (!m_filter)
            {
                return {};
    ...
    bool DrawCustomLayout(ImGuiContext * ctx, float zoom, ImVec2 origin, ImGui::ImCurveEdit::Curve * key, bool embedded) override
    {
        ...
        ImGui::SameLine(setting_offset);  if (ImGui::Button(ICON_RESET "##reset_fov##Orthographic2Equidistance")) { _fov = 180.f; changed = true; }
        if (!embedded) ImGui::ShowTooltipOnHover("Reset");
        ImGui::SliderFloat("Center X", &_cx, 0.f, 1.f, "%.3f", flags);
        ImGui::SameLine(setting_offset);  if (ImGui::Button(ICON_RESET "##reset_cx##Orthographic2Equidistance")) { _cx = .5f; changed = true; }
        if (!embedded) ImGui::ShowTooltipOnHover("Reset");
        ImGui::SliderFloat("Center Y", &_cy, 0.f, 1.f, "%.3f", flags);
        ImGui::SameLine(setting_offset);  if (ImGui::Button(ICON_RESET "##reset_cy##Orthographic2Equidistance")) { _cy = .5f; changed = true; }
        if (!embedded) ImGui::ShowTooltipOnHover("Reset");
...
private:
    ...
    ImGui::Orthographic2Equidistance_vulkan * m_filter {nullptr};
};
} //namespace BluePrint

BP_NODE_DYNAMIC_WITH_NAME(Orthographic2EquidistanceNode, "Orthographic to Equidistance", "CodeWin", NODE_VERSION, VERSION_BLUEPRINT_API, BluePrint::NodeType::External, BluePrint::NodeStyle::Default, "Filter#Video#Fisheye")
