...
namespace BluePrint
{
struct Fish2SphereNode final : Node
{
    BP_NODE_WITH_NAME(Fish2SphereNode, "Fish2Sphere", "CodeWin", NODE_VERSION, VERSION_BLUEPRINT_API, NodeType::External, NodeStyle::Default, "Filter#Video#Fisheye")
    Fish2SphereNode(BP* blueprint): Node(blueprint) { m_Name = "Fish to Sphere"; m_HasCustomLayout = true; m_Skippable = true; }
    ~Fish2SphereNode()
    {
    ...
    FlowPin Execute(Context& context, FlowPin& entryPoint, bool threading = false) override
    {
        ...
        if (!mat_in.empty())
        {
            ...
            {
                if (m_filter) { delete m_filter; m_filter = nullptr; }
                m_filter = new ImGui::Fish2Sphere_vulkan(gpu);
            }
            if (!m_filter)
            {
                return {};
    ...
    bool DrawCustomLayout(ImGuiContext * ctx, float zoom, ImVec2 origin, ImGui::ImCurveEdit::Curve * key, bool embedded) override
    {
        ...
        ImGui::SameLine(setting_offset);  if (ImGui::Button(ICON_RESET "##reset_fov##Fish2Sphere")) { _fov = 180.f; changed = true; }
        if (!embedded) ImGui::ShowTooltipOnHover("Reset");
        ImGui::SliderFloat("Center X", &_cx, 0.f, 1.f, "%.3f", flags);
        ImGui::SameLine(setting_offset);  if (ImGui::Button(ICON_RESET "##reset_cx##Fish2Sphere")) { _cx = .5f; changed = true; }
        if (!embedded) ImGui::ShowTooltipOnHover("Reset");
        ImGui::SliderFloat("Center Y", &_cy, 0.f, 1.f, "%.3f", flags);
        ImGui::SameLine(setting_offset);  if (ImGui::Button(ICON_RESET "##reset_cy##Fish2Sphere")) { _cy = .5f; changed = true; }
        if (!embedded) ImGui::ShowTooltipOnHover("Reset");
...
private:
    ...
    ImGui::Fish2Sphere_vulkan * m_filter {nullptr};
};
} //namespace BluePrint

BP_NODE_DYNAMIC_WITH_NAME(Fish2SphereNode, "Fish to Sphere", "CodeWin", NODE_VERSION, VERSION_BLUEPRINT_API, BluePrint::NodeType::External, BluePrint::NodeStyle::Default, "Filter#Video#Fisheye")
