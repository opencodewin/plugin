...
#define ICON_VIEW   ICON_MD_VISIBILITY
#else
...
#define NODE_VERSION    0x01050000
...
struct CustomShaderNode final : Node
{
    ...
    void SetErrorPoint(int start_lines, int filter_lines)
    {
        if (!m_compile_log.empty())
        {
            ...
            TextEditor::ErrorMarkers fullcode_markers;
                    ...
                    if (this_word.compare("ERROR") == 0)
                    {
                        ...
                        if ((end = logs[i].find(":", start)) != string::npos)
                        {
                            ...
                            if ((end = logs[i].find(":", start)) != string::npos)
                            {
                                ...
                                {
                                    markers.insert(std::make_pair<int, std::string>(col - start_lines, msg.c_str()));
                                    fullcode_markers.insert(std::make_pair<int, std::string>((int)col, msg.c_str()));
                                }
                            }
                        }
                    }
            ...
            m_editor_preview.SetErrorMarkers(fullcode_markers);
        }
    }
    ...
    bool DrawShaderEditor()
    {
        ...
        ImGui::SameLine();
        if (ImGui::Button(ICON_VIEW "##custom_shader"))
        {
            auto full_code = m_program_start + m_program_filter;
            m_editor_preview.SetText(full_code);
            ImGui::OpenPopup("Full code preview##custom_shader", ImGuiPopupFlags_AnyPopup);
        } ImGui::ShowTooltipOnHover("Show full code");
        ImGui::SameLine(); ImGui::Spacing();ImGui::SameLine();
        ...
        if (ImGui::BeginPopupModal("Full code preview##custom_shader"))
        {
            m_editor_preview.SetReadOnly(true);
            auto window_width = ImGui::GetContentRegionAvail().x;
            auto window_height = ImGui::GetWindowSize().y - 64;
            m_editor_preview.Render("VulkanShaderPreview", ImVec2(window_width, window_height));
            ImGui::Indent((window_width - 40.0f) * 0.5f);
        ...
        if (ImGuiFileDialog::Instance()->Display("##CustomShader_FileDlg", ImGuiWindowFlags_NoCollapse, minSize, maxSize))
        {
            if (ImGuiFileDialog::Instance()->IsOk())
            {
                ...
                else if (userDatas.compare("Save Source") == 0)
                {
                    ...
                    //os << m_program_start << m_program_filter << std::endl;
                    os.close();
                }
            }
