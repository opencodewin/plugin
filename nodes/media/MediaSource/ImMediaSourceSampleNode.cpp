...
namespace BluePrint
{
struct MediaSourceSampleNode final : Node
{
    struct media_stream
    {
        int                 m_index {-1};
    ...
    BP_NODE_WITH_NAME(MediaSourceSampleNode, "Media Source Sample", "CodeWin", NODE_VERSION, VERSION_BLUEPRINT_API, NodeType::External, NodeStyle::Default, "Media")
    MediaSourceSampleNode(BP* blueprint): Node(blueprint)
    {
        m_Name = "Media Source Sample";
    ...
    void OpenMedia()
    {
        ...
        if (!url && !ifmt) return;
        // Open New file
        if (avformat_open_input(&m_fmt_ctx, url, ifmt, &options) < 0) 
    ...
    bool DrawCustomLayout(ImGuiContext * ctx, float zoom, ImVec2 origin, ImGui::ImCurveEdit::Curve * key, bool embedded) override
    {
        ...
        for (auto stream : m_streams)
        {
            if (stream->m_type == AVMEDIA_TYPE_VIDEO)
            {
                ...
                ImGui::Text("            %d bit depth", stream->m_stream->codecpar->bits_per_raw_sample > 0 ? stream->m_stream->codecpar->bits_per_raw_sample : 8);
...
BP_NODE_DYNAMIC_WITH_NAME(MediaSourceSampleNode, "Media Source Sample", "CodeWin", NODE_VERSION, VERSION_BLUEPRINT_API, BluePrint::NodeType::External, BluePrint::NodeStyle::Default, "Media")
