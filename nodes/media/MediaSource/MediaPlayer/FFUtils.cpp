...
bool HwFrameToSwFrame(AVFrame* swfrm, const AVFrame* hwfrm)
{
    ...
    if (fferr < 0)
    {
        // cerr << "av_hwframe_map() FAILED! fferr=" << fferr << "." << endl;
        av_frame_unref(swfrm);
        swfrm->format = (int)AV_PIX_FMT_NONE;
        fferr = av_hwframe_transfer_data(swfrm, hwfrm, 0);
        if (fferr < 0)
        {
            cerr << "av_hwframe_map and av_hwframe_transfer_data() FAILED! fferr=" << fferr << "." << endl;
            return false;
        }
    }
...
bool MakeAVFrameCopy(AVFrame* dst, const AVFrame* src)
{
    ...
    {
        cerr << "av_frame_get_buffer() FAILED! fferr = " << fferr << "." << endl;
        return false;
    }
    if ((fferr = av_frame_copy(dst, src)) < 0)
    {
        cerr << "av_frame_copy() FAILED! fferr = " << fferr << "." << endl;
        return false;
    }
...
bool ConvertImMatToAVFrame(const ImGui::ImMat& vmat, AVFrame* avfrm, int64_t pts)
{
    ...
    {
        cerr << "Input ImMat is NOT a CPU mat!" << endl;
        return false;
    }
    AVPixelFormat cvtPixfmt = ConvertColorFormatToPixelFormat(vmat.color_format, vmat.type);
    if (cvtPixfmt < 0)
    {
        cerr << "FAILED to convert ImColorFormat " << vmat.color_format << " and ImDataType " << vmat.type << " to AVPixelFormat!" << endl;
        return false;
    }
    ...
    if (fferr < 0)
    {
        cerr << "FF api 'av_frame_get_buffer' failed! return code is " << fferr << ".";
        return false;
    }
...
SelfFreeAVFramePtr ImMatWrapper_AVFrame::GetWrapper(int64_t pts)
{
    ...
    if (!avfrm)
    {
        cerr << "FAILED to allocate new AVFrame instance by 'av_frame_alloc()'!" << endl;
        return nullptr;
    }
    if (m_isVideo)
    {
        ...
        {
            cerr << "CANNOT find suitable AVPixelFormat for ImColorFormat " << m_mat.color_format << "!" << endl;
            return nullptr;
        }
