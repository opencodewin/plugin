...
struct ColorCurveNode final : Node
{
    ...
    FlowPin Execute(Context& context, FlowPin& entryPoint, bool threading = false) override
    {
        ...
        if (!mat_in.empty())
        {
            ...
            m_NodeTimeMs = m_filter->filter(mat_in, im_RGB, mColorCurve.mMat_curve);
    ...
    bool DrawCustomLayout(ImGuiContext * ctx, float zoom, ImVec2 origin, ImGui::ImCurveEdit::Curve * key, bool embedded) override
    {
        ...
        bool changed = false;
        changed = ImGui::ShowColorCurve("Color Curve##demo", scope_view_size, mColorCurve, mHistogramScale, mMat_histogram, mHistogramLog);
        return changed;
    }
    ...
    int Load(const imgui_json::value& value) override
    {
        ...
        if (value.contains("histogram_index"))
        {
            ...
            if (val.is_number()) 
                mColorCurve.mEditIndex = val.get<imgui_json::number>();
        }
        ...
        if (imgui_json::GetPtrTo(value, "curve_y", pointArrayY))
        {
            ...
            for (auto& point : *pointArrayY)
            {
                ...
                if (point.contains("Point"))
                {
                    ...
                    if (val.is_vec2()) mColorCurve.mCurve[0].push_back(val.get<imgui_json::vec2>());
                }
            }
        }
        ...
        if (imgui_json::GetPtrTo(value, "curve_r", pointArrayR))
        {
            ...
            for (auto& point : *pointArrayR)
            {
                ...
                {
                    auto& val = point["Point"];
                    if (val.is_vec2()) mColorCurve.mCurve[1].push_back(val.get<imgui_json::vec2>());
                }
            }
        }
        ...
        if (imgui_json::GetPtrTo(value, "curve_g", pointArrayG))
        {
            ...
            for (auto& point : *pointArrayG)
            {
                ...
                {
                    auto& val = point["Point"];
                    if (val.is_vec2()) mColorCurve.mCurve[2].push_back(val.get<imgui_json::vec2>());
                }
            }
        }
        ...
        if (imgui_json::GetPtrTo(value, "curve_b", pointArrayB))
        {
            ...
            for (auto& point : *pointArrayB)
            {
                ...
                {
                    auto& val = point["Point"];
                    if (val.is_vec2()) mColorCurve.mCurve[3].push_back(val.get<imgui_json::vec2>());
                }
            }
        }

        mColorCurve.SetCurveMat();
        return ret;
    }
    ...
    void Save(imgui_json::value& value, std::map<ID_TYPE, ID_TYPE> MapID) override
    {
        ...
        value["histogram_index"] = imgui_json::number(mColorCurve.mEditIndex);
        value["histogram_log"] = imgui_json::boolean(mHistogramLog);
        value["curve_tension"] = imgui_json::number(mColorCurve.mTension);

        imgui_json::value curve_y;
        for (int i = 0; i < mColorCurve.mCurve[0].size(); i++)
        {
            imgui_json::value point;
            point["Point"] = imgui_json::vec2(mColorCurve.mCurve[0][i]);
            curve_y.push_back(point);
        }
        value["curve_y"] = curve_y;

        imgui_json::value curve_r;
        for (int i = 0; i < mColorCurve.mCurve[1].size(); i++)
        {
            imgui_json::value point;
            point["Point"] = imgui_json::vec2(mColorCurve.mCurve[1][i]);
            curve_r.push_back(point);
        }
        value["curve_r"] = curve_r;

        imgui_json::value curve_g;
        for (int i = 0; i < mColorCurve.mCurve[2].size(); i++)
        {
            imgui_json::value point;
            point["Point"] = imgui_json::vec2(mColorCurve.mCurve[2][i]);
            curve_g.push_back(point);
        }
        value["curve_g"] = curve_g;

        imgui_json::value curve_b;
        for (int i = 0; i < mColorCurve.mCurve[3].size(); i++)
        {
            imgui_json::value point;
            point["Point"] = imgui_json::vec2(mColorCurve.mCurve[3][i]);
